...
from config import BINANCE_WEBSOCKET_URL, CURRENCY_CACHE_TIME
...
async def set_currency(
    ...
    async with session.begin():
        ...
        stmt = (
            update(Currency)
            .values(**currency_data.model_dump())
            .where(
                (Currency.wallet_id == wallet_id)
    ...
    async with session.begin():
        if transaction_data.quantity <= 0:
            return HTTPException(
                status_code=400,
                detail={
                    "message": f"You can't buy 0 or less {transaction_data.currency} coins"
                },
            )
        ...
        balance_result = await session.execute(
            select(Currency.quantity).where(
                (Currency.wallet_id == wallet_id) & (Currency.name == "USDT")
            )
        )
        ...
        new_currency_data = {"name": transaction_currency, "quantity": quantity}
        ...
        if transaction_type not in TRANSACTION_OPERATIONS:
            return HTTPException(
                status_code=400, detail={"message": "Incorrect operation"}
            )
        ...
        if transaction_type == TRANSACTION_OPERATIONS[0]:
            if balance < price:
                return HTTPException(
                    status_code=400,
                    detail={
                        "message": f"Your balance is less than {transaction_currency} coin's price"
                    },
                )
            ...
            currency = await session.execute(
                select(Currency).where(Currency.name == transaction_currency)
            )
            ...
            if currency.first() is not None:
                ...
                await set_currency(
                    user_id=user_id,
                    currency_data=CurrencyChangeSchema(**new_currency_data),
                )
            else:
        ...
        if transaction_type == TRANSACTION_OPERATIONS[1]:
            if balance <= 0:
                return HTTPException(
                    status_code=400,
                    detail={f"There is not enough balance in your wallet"},
                )
            ...
            if currency_quantity is None or currency_quantity <= 0:
                return HTTPException(
                    status_code=400,
                    detail={
                        "message": f"You have not enough {transaction_currency} coins"
                    },
                )
        ...
        new_balance_data = {"name": "USDT", "quantity": balance}
        ...
        async with websockets.connect(uri=connection_url) as ws:
            while True:
                ...
                await asyncio.to_thread(
                    process_json_list, json_list=json_list, coin_name=coin_name
                )
